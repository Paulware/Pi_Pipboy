...
<script>
  ...
  function forSale (checked,ID) {
     var sql = "Update inventory set ForSale=0 Where ID=" + ID;
     if (checked) {
        sql = "Update inventory set ForSale=1 Where ID=" + ID;
     } 
     window.location.href = 'query.php?sql=' + escape(sql);
  }  
  function archive (checked,ItemId) {  
     var sql = "Update inventory set InVault=0 Where ID=" + ID;
     if (checked) {
        sql = "Update inventory set InVault=1 Where ID=" + ID;
     } 
     window.location.href = 'query.php?sql=' + escape(sql);     
  }  
</script>
...
<?php 
    ...
    while ($row = mysql_fetch_assoc ($result)) {		
       ...
       if ($ForSale == 1) {
         print ("<td align=\"center\"><input type=\"checkbox\" checked onchange=\"forSale(this.checked, $ID);\"></td>\n" );
       } else {
         print ("<td align=\"center\"><input type=\"checkbox\" onchange=\"forSale(this.checked, $ID);\"></td>\n" );
       }       
       ...
       if ($InVault == 1) {
         print ("<td align=\"center\"><input type=\"checkbox\" checked onchange=\"archive(this.checked, $ID);\"></td>\n" );
       } else {
         print ("<td align=\"center\"><input type=\"checkbox\" onchange=\"archive(this.checked, $ID);\"></td>\n" );
       }       
